...
module ace_cut #(
    ...
    parameter bit  BypassAck  = Bypass,
    // ACE channel structs
    ...
    if (BypassAck) begin : gen_xack_bypass
        ...
        always_ff @(posedge clk_i or negedge rst_ni) begin
            if (!rst_ni) begin
                ...
                wack_q <= slv_req_i.wack;
                rack_q <= slv_req_i.rack;
            end
        end

        assign mst_req_o.wack = wack_q;
        assign mst_req_o.rack = rack_q;
    end
endmodule
...
module ace_cut_intf #(
    ...
    parameter bit          BYPASS_ACK = BYPASS,
    // The address width.
