...
#(
    ...
    localparam int unsigned scoreboardEntries         = ccuCfg.u.numShareableTransactions,
...
logic              [scoreboardEntries-1:0]                                       valid_q;
logic              [scoreboardEntries-1:0]                                       valid_d;
scoreboard_entry_t [scoreboardEntries-1:0]                                       entry_q;
scoreboard_entry_t [scoreboardEntries-1:0]                                       entry_d;
logic              [scoreboardEntries-1:0]                                       address_hit;
logic              [scoreboardEntries-1:0]                                       dealloc_id_hit;
logic              [scoreboardEntryIndexWidth-1:0]                               alloc_entry;
...
always_comb begin : alloc_entry_comb
    alloc_entry = '0;
    for (int unsigned e = 0; e < scoreboardEntries; e++) begin
        if (!valid_q[e]) begin
            alloc_entry = e;
            break;
        end
...
for (genvar e = 0; e < scoreboardEntries; e++) begin : gen_entry
    ...
    assign alloc             = alloc_i && alloc_entry == e;
    assign dealloc           = dealloc_i[subordinate_index] && dealloc_entry_i[subordinate_index] == e;
...
onehot_to_bin #(
    .ONEHOT_WIDTH (scoreboardEntries)
) u_onehot_to_bin (
